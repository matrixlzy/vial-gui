...
KEYCODES_BASIC_NUMPAD = [
    K("KC_NUMLOCK", "Num\nLock\n\n数字锁定键", recorder_alias=["num lock"], alias=["KC_NLCK"]),
    K("KC_KP_SLASH", "/\n\n除号键", alias=["KC_PSLS"]),
    K("KC_KP_ASTERISK", "*\n\n乘号键", alias=["KC_PAST"]),
    K("KC_KP_MINUS", "-\n\n减号键", alias=["KC_PMNS"]),
    K("KC_KP_PLUS", "+\n\n加号键", alias=["KC_PPLS"]),
...
KEYCODES_BASIC_NAV = [
    K("KC_PSCREEN", "Print\nScreen\n\n屏幕截图键", alias=["KC_PSCR"]),
    K("KC_SCROLLLOCK", "Scroll\nLock\n\n滚动锁定键", recorder_alias=["scroll lock"], alias=["KC_SLCK", "KC_BRMD"]),
    K("KC_PAUSE", "Pause\n\n暂停键", recorder_alias=["pause", "break"], alias=["KC_PAUS", "KC_BRK", "KC_BRMU"]),
    K("KC_INSERT", "Insert\n\n插入键", recorder_alias=["insert"], alias=["KC_INS"]),
    K("KC_HOME", "Home\n\n行首键", recorder_alias=["home"]),
    K("KC_PGUP", "Page\nUp\n\n上翻页键", recorder_alias=["page up"]),
    K("KC_DELETE", "Del\n\n删除键", recorder_alias=["delete"], alias=["KC_DEL"]),
    K("KC_END", "End\n\n行尾键", recorder_alias=["end"]),
    K("KC_PGDOWN", "Page\nDown\n\n下翻页键", recorder_alias=["page down"], alias=["KC_PGDN"]),
    K("KC_RIGHT", "Right\n\n方向右", recorder_alias=["right"], alias=["KC_RGHT"]),
    K("KC_LEFT", "Left\n\n方向左", recorder_alias=["left"]),
    K("KC_DOWN", "Down\n\n方向下", recorder_alias=["down"]),
    K("KC_UP", "Up\n\n方向上", recorder_alias=["up"]),
]
...
KEYCODES_BASIC = [
    ...
    K("KC_ENTER", "Enter\n\n回车键", recorder_alias=["enter"], alias=["KC_ENT"]),
    K("KC_ESCAPE", "Esc\n\n退出键", recorder_alias=["esc"], alias=["KC_ESC"]),
    K("KC_BSPACE", "Bksp\n\n退格键", recorder_alias=["backspace"], alias=["KC_BSPC"]),
    K("KC_TAB", "Tab\n\n跳格键", recorder_alias=["tab"]),
    K("KC_SPACE", "Space\n\n空格键", recorder_alias=["space"], alias=["KC_SPC"]),
    ...
    K("KC_CAPSLOCK", "Caps\nLock\n\n大写锁定键", recorder_alias=["caps lock"], alias=["KC_CLCK", "KC_CAPS"]),
    ...
    K("KC_APPLICATION", "Menu\n\n菜单键", recorder_alias=["menu", "left menu", "right menu"], alias=["KC_APP"]),
    K("KC_LCTRL", "LCtrl\n\n左控制键", recorder_alias=["left ctrl", "ctrl"], alias=["KC_LCTL"]),
    K("KC_LSHIFT", "LShift\n\n左上档键", recorder_alias=["left shift", "shift"], alias=["KC_LSFT"]),
    K("KC_LALT", "LAlt\n\n左换档键", recorder_alias=["alt"], alias=["KC_LOPT"]),
    K("KC_LGUI", "LGui\n\n左Win建", recorder_alias=["left windows", "windows"], alias=["KC_LCMD", "KC_LWIN"]),
    K("KC_RCTRL", "RCtrl\n\n右控制键", recorder_alias=["right ctrl"], alias=["KC_RCTL"]),
    K("KC_RSHIFT", "RShift\n\n右上档键", recorder_alias=["right shift"], alias=["KC_RSFT"]),
    K("KC_RALT", "RAlt\n\n右换档键", alias=["KC_ALGR", "KC_ROPT"]),
    K("KC_RGUI", "RGui\n\n右Win键", recorder_alias=["right windows"], alias=["KC_RCMD", "KC_RWIN"]),
]
...
KEYCODES_MODIFIERS = [
    ...
    K("LSFT_T(kc)", "LSft_T\n(kc)", "按住时为左Shift，点按时为kc", masked=True),
    K("LCTL_T(kc)", "LCtl_T\n(kc)", "按住时为左Control，点按时为kc", masked=True),
    K("LALT_T(kc)", "LAlt_T\n(kc)", "按住时为左Alt，点按时为kc", masked=True),
    K("LGUI_T(kc)", "LGui_T\n(kc)", "按住时为左GUI（Win/Cmd），点按时为kc", masked=True),
    K("RSFT_T(kc)", "RSft_T\n(kc)", "按住时为右Shift，点按时为kc", masked=True),
    K("RCTL_T(kc)", "RCtl_T\n(kc)", "按住时为右Control，点按时为kc", masked=True),
    K("RALT_T(kc)", "RAlt_T\n(kc)", "按住时为右Alt，点按时为kc", masked=True),
    K("RGUI_T(kc)", "RGui_T\n(kc)", "按住时为右GUI（Win/Cmd），点按时为kc", masked=True),
    K("C_S_T(kc)", "LCS_T\n(kc)", "按住时为左Control+左Shift，点按时为kc", masked=True, alias=["LCS_T(kc)"] ),
    K("LCA_T(kc)", "LCA_T\n(kc)", "按住时为左Control+左Alt，点按时为kc", masked=True),
    K("LCG_T(kc)", "LCG_T\n(kc)", "按住时为左Control+左GUI，点按时为kc", masked=True),
    K("LSA_T(kc)", "LSA_T\n(kc)", "按住时为左Shift+左Alt，点按时为kc", masked=True),
    K("LAG_T(kc)", "LAG_T\n(kc)", "按住时为左Alt+左GUI，点按时为kc", masked=True),
    K("SGUI_T(kc)", "LSG_T\n(kc)", "按住时为左GUI+左Shift，点按时为kc", masked=True, alias=["LSG_T(kc)"]),
    K("LCAG_T(kc)", "LCAG_T\n(kc)", "按住时为左Control+左Alt+左GUI，点按时为kc", masked=True),
    K("RCG_T(kc)", "RCG_T\n(kc)", "按住时为右Control+右GUI，点按时为kc", masked=True),
    K("RCAG_T(kc)", "RCAG_T\n(kc)", "按住时为右Control+右Alt+右GUI，点按时为kc", masked=True),
    K("MEH_T(kc)", "Meh_T\n(kc)", "按住时为左Control+左Shift+左Alt，点按时为kc", masked=True),
    K("ALL_T(kc)", "ALL_T\n(kc)", "按住时为左Control+左Shift+左Alt+左GUI，点按时为kc", masked=True),

    K("KC_GESC", "~\nEsc", "通常为Esc，但当Shift或GUI按下时为~"),
    K("KC_LSPO", "LS\n(", "按住时为左Shift，点按时为("),
    K("KC_RSPC", "RS\n)", "按住时为右Shift，点按时为)"),
    K("KC_LCPO", "LC\n(", "按住时为左Control，点按时为("),
    K("KC_RCPC", "RC\n)", "按住时为右Control，点按时为)"),
    K("KC_LAPO", "LA\n(", "按住时为左Alt，点按时为("),
    K("KC_RAPC", "RA\n)", "按住时为右Alt，点按时为)"),
    K("KC_SFTENT", "RS\nEnter", "按住时为右Shift，点按时为Enter"),
]

KEYCODES_QUANTUM = [
    K("MAGIC_SWAP_CONTROL_CAPSLOCK", "Swap\nCtrl\nCaps", "交换左 Control 和 Caps Lock", alias=["CL_SWAP"]),
    K("MAGIC_UNSWAP_CONTROL_CAPSLOCK", "Unswap\nCtrl\nCaps", "还原左 Control 和 Caps Lock", alias=["CL_NORM"]),
    K("MAGIC_CAPSLOCK_TO_CONTROL", "Caps\nto\nCtrl", "将 Caps Lock 当作 Control 使用", alias=["CL_CTRL"]),
    K("MAGIC_UNCAPSLOCK_TO_CONTROL", "Caps\nnot to\nCtrl", "停止将 Caps Lock 当作 Control", alias=["CL_CAPS"]),
    K("MAGIC_SWAP_LCTL_LGUI", "Swap\nLCtl\nLGui", "交换左 Control 和左 GUI", alias=["LCG_SWP"]),
    K("MAGIC_UNSWAP_LCTL_LGUI", "Unswap\nLCtl\nLGui", "还原左 Control 和左 GUI", alias=["LCG_NRM"]),
    K("MAGIC_SWAP_RCTL_RGUI", "Swap\nRCtl\nRGui", "交换右 Control 和右 GUI", alias=["RCG_SWP"]),
    K("MAGIC_UNSWAP_RCTL_RGUI", "Unswap\nRCtl\nRGui", "还原右 Control 和右 GUI", alias=["RCG_NRM"]),
    K("MAGIC_SWAP_CTL_GUI", "Swap\nCtl\nGui", "交换两侧的 Control 和 GUI", alias=["CG_SWAP"]),
    K("MAGIC_UNSWAP_CTL_GUI", "Unswap\nCtl\nGui", "还原两侧的 Control 和 GUI", alias=["CG_NORM"]),
    K("MAGIC_TOGGLE_CTL_GUI", "Toggle\nCtl\nGui", "切换两侧 Control 和 GUI 的交换状态", alias=["CG_TOGG"]),
    K("MAGIC_SWAP_LALT_LGUI", "Swap\nLAlt\nLGui", "交换左 Alt 和左 GUI", alias=["LAG_SWP"]),
    K("MAGIC_UNSWAP_LALT_LGUI", "Unswap\nLAlt\nLGui", "还原左 Alt 和左 GUI", alias=["LAG_NRM"]),
    K("MAGIC_SWAP_RALT_RGUI", "Swap\nRAlt\nRGui", "交换右 Alt 和右 GUI", alias=["RAG_SWP"]),
    K("MAGIC_UNSWAP_RALT_RGUI", "Unswap\nRAlt\nRGui", "还原右 Alt 和右 GUI", alias=["RAG_NRM"]),
    K("MAGIC_SWAP_ALT_GUI", "Swap\nAlt\nGui", "交换两侧的 Alt 和 GUI", alias=["AG_SWAP"]),
    K("MAGIC_UNSWAP_ALT_GUI", "Unswap\nAlt\nGui", "还原两侧的 Alt 和 GUI", alias=["AG_NORM"]),
    K("MAGIC_TOGGLE_ALT_GUI", "Toggle\nAlt\nGui", "切换两侧 Alt 和 GUI 的交换状态", alias=["AG_TOGG"]),
    K("MAGIC_NO_GUI", "GUI\nOff", "禁用 GUI 键", alias=["GUI_OFF"]),
    K("MAGIC_UNNO_GUI", "GUI\nOn", "启用 GUI 键", alias=["GUI_ON"]),
    K("MAGIC_TOGGLE_GUI", "GUI\nToggle", "切换 GUI 键的启用状态", alias=["GUI_TOGG"]),
    K("MAGIC_SWAP_GRAVE_ESC", "Swap\n`\nEsc", "交换 ` 和 Escape", alias=["GE_SWAP"]),
    K("MAGIC_UNSWAP_GRAVE_ESC", "Unswap\n`\nEsc", "还原 ` 和 Escape", alias=["GE_NORM"]),
    K("MAGIC_SWAP_BACKSLASH_BACKSPACE", "Swap\n\\\nBS", "交换 \\ 和 Backspace", alias=["BS_SWAP"]),
    K("MAGIC_UNSWAP_BACKSLASH_BACKSPACE", "Unswap\n\\\nBS", "还原 \\ 和 Backspace", alias=["BS_NORM"]),
    K("MAGIC_HOST_NKRO", "NKRO\nOn", "启用 N 键无冲", alias=["NK_ON"]),
    K("MAGIC_UNHOST_NKRO", "NKRO\nOff", "禁用 N 键无冲", alias=["NK_OFF"]),
    K("MAGIC_TOGGLE_NKRO", "NKRO\nToggle", "切换 N 键无冲状态", alias=["NK_TOGG"]),
    K("MAGIC_EE_HANDS_LEFT", "EEH\nLeft", "将分体键盘的主半区设为左手（用于 EE_HANDS）",
      alias=["EH_LEFT"]),
    K("MAGIC_EE_HANDS_RIGHT", "EEH\nRight", "将分体键盘的主半区设为右手（用于 EE_HANDS）",
      alias=["EH_RGHT"]),

    K("AU_ON", "Audio\nON", "开启音频模式"),
    K("AU_OFF", "Audio\nOFF", "关闭音频模式"),
    K("AU_TOG", "Audio\nToggle", "切换音频模式"),
    K("CLICKY_TOGGLE", "Clicky\nToggle", "切换滴答音效模式", alias=["CK_TOGG"]),
    K("CLICKY_UP", "Clicky\nUp", "提高滴答音频率", alias=["CK_UP"]),
    K("CLICKY_DOWN", "Clicky\nDown", "降低滴答音频率", alias=["CK_DOWN"]),
    K("CLICKY_RESET", "Clicky\nReset", "重置滴答音频率为默认值", alias=["CK_RST"]),
    K("MU_ON", "Music\nOn", "开启音乐模式"),
    K("MU_OFF", "Music\nOff", "关闭音乐模式"),
    K("MU_TOG", "Music\nToggle", "切换音乐模式"),
    K("MU_MOD", "Music\nCycle", "循环切换音乐模式"),
    ...
    K("KC_ASON", "Auto-\nshift\nOn", "开启自动移位功能"),
    K("KC_ASOFF", "Auto-\nshift\nOff", "关闭自动移位功能"),
    K("KC_ASTG", "Auto-\nshift\nToggle", "切换自动移位功能的开关状态"),

    K("CMB_ON", "Combo\nOn", "开启组合键功能"),
    K("CMB_OFF", "Combo\nOff", "关闭组合键功能"),
    K("CMB_TOG", "Combo\nToggle", "切换组合键功能的开关"),

    K("QK_CAPS_WORD_TOGGLE", "Caps\nWord", "在当前单词结束前将字母大写", alias=["CW_TOGG"], requires_feature="caps_word"),
    K("QK_REPEAT_KEY", "Repeat", "重复上一次按下的键", alias=["QK_REP"], requires_feature="repeat_key"),
    K("QK_ALT_REPEAT_KEY", "Alt\nRepeat", "Alt 重复上一次按下的键", alias=["QK_AREP"], requires_feature="repeat_key"),
]
...
KEYCODES_BACKLIGHT = [
    ...
    K("BL_OFF", "BL Off", "关闭背光"),
    K("BL_INC", "BL +", "增加背光亮度等级"),
    K("BL_DEC", "BL - ", "降低背光亮度等级"),

    K("RGB_TOG", "RGB\nToggle", "切换 RGB 灯效的开关状态"),
    K("RGB_MOD", "RGB\nMode +", "下一个 RGB 模式"),
    K("RGB_RMOD", "RGB\nMode -", "上一个 RGB 模式"),
    K("RGB_HUI", "Hue +", "增加色调"),
    K("RGB_HUD", "Hue -", "减少色调"),
    ...
    K("RGB_SPI", "Effect +", "加快 RGB 特效速度"),
    K("RGB_SPD", "Effect -", "减慢 RGB 特效速度"),
    K("RGB_M_P", "RGB\nMode P", "RGB 模式：单色常亮"),
    K("RGB_M_B", "RGB\nMode B", "RGB 模式：呼吸"),
    K("RGB_M_R", "RGB\nMode R", "RGB 模式：彩虹"),
    K("RGB_M_SW", "RGB\nMode SW", "RGB 模式：旋涡"),
    K("RGB_M_SN", "RGB\nMode SN", "RGB 模式：贪吃蛇"),
    K("RGB_M_K", "RGB\nMode K", "RGB 模式：流光（霹雳游侠效果）"),
    K("RGB_M_X", "RGB\nMode X", "RGB 模式：圣诞"),
    K("RGB_M_G", "RGB\nMode G", "RGB 模式：渐变"),
    K("RGB_M_T", "RGB\nMode T", "RGB 模式：测试"),
...
KEYCODES_MEDIA = [
    ...
    K("KC_PWR", "Power\n\n电源", "系统关机", alias=["KC_SYSTEM_POWER"]),
    K("KC_SLEP", "Sleep\n\n睡眠", "系统睡眠", alias=["KC_SYSTEM_SLEEP"]),
    K("KC_WAKE", "Wake\n\n唤醒", "系统唤醒", alias=["KC_SYSTEM_WAKE"]),
    K("KC_EXEC", "Exec\n\n执行", "执行", alias=["KC_EXECUTE"]),
    K("KC_HELP", "Help\n\n帮助"),
    K("KC_SLCT", "Select\n\n选择", alias=["KC_SELECT"]),
    K("KC_STOP", "Stop\n\n停止"),
    K("KC_AGIN", "Again\n\n再次", alias=["KC_AGAIN"]),
    ...
    K("KC_WBAK", "Browser\nBack", "浏览器后退(Windows)", alias=["KC_WWW_BACK"]),
    ...
    K("KC_BRIU", "Bright.\nUp", "提高屏幕亮度(笔记本)", alias=["KC_BRIGHTNESS_UP"]),
    K("KC_BRID", "Bright.\nDown", "降低屏幕亮度(笔记本)", alias=["KC_BRIGHTNESS_DOWN"]),

    K("KC_MPRV", "Media\nPrev", "上一首", alias=["KC_MEDIA_PREV_TRACK"]),
    K("KC_MNXT", "Media\nNext", "下一首", alias=["KC_MEDIA_NEXT_TRACK"]),
    K("KC_MUTE", "Mute", "静音", alias=["KC_AUDIO_MUTE"]),
    K("KC_VOLD", "Vol -", "降低音量", alias=["KC_AUDIO_VOL_DOWN"]),
    K("KC_VOLU", "Vol +", "增加音量", alias=["KC_AUDIO_VOL_UP"]),
    K("KC__VOLDOWN", "Vol -\nAlt", "降低音量(备用)"),
    K("KC__VOLUP", "Vol +\nAlt", "增加音量(备用)"),
    K("KC_MSTP", "Media\nStop","停止", alias=["KC_MEDIA_STOP"]),
    K("KC_MPLY", "Media\nPlay", "播放/暂停", alias=["KC_MEDIA_PLAY_PAUSE"]),
    K("KC_MRWD", "Prev\nTrack\n(macOS)", "上一曲 / 倒带(macOS)", alias=["KC_MEDIA_REWIND"]),
    K("KC_MFFD", "Next\nTrack\n(macOS)", "下一曲 / 快进(macOS)", alias=["KC_MEDIA_FAST_FORWARD"]),
    K("KC_EJCT", "Eject", "弹出(macOS)", alias=["KC_MEDIA_EJECT"]),
    ...
    K("KC_WH_U", "Mouse\nWheel\nUp","滚轮前滚", alias=["KC_MS_WH_UP"]),
    K("KC_WH_D", "Mouse\nWheel\nDown","滚轮后滚", alias=["KC_MS_WH_DOWN"]),
    K("KC_WH_L", "Mouse\nWheel\nLeft","滚轮左滚", alias=["KC_MS_WH_LEFT"]),
    K("KC_WH_R", "Mouse\nWheel\nRight","滚轮右滚", alias=["KC_MS_WH_RIGHT"]),
    K("KC_ACL0", "Mouse\nAccel\n0", "设置鼠标加速到 0", alias=["KC_MS_ACCEL0"]),
    K("KC_ACL1", "Mouse\nAccel\n1", "设置鼠标加速到 1", alias=["KC_MS_ACCEL1"]),
    K("KC_ACL2", "Mouse\nAccel\n2", "设置鼠标加速到 2", alias=["KC_MS_ACCEL2"]),

    K("KC_LCAP", "Locking\nCaps", "锁定大写锁定键", alias=["KC_LOCKING_CAPS"]),
    K("KC_LNUM", "Locking\nNum", "锁定数字锁定键", alias=["KC_LOCKING_NUM"]),
    K("KC_LSCR", "Locking\nScroll", "锁定滚动锁定键", alias=["KC_LOCKING_SCROLL"]),
]
