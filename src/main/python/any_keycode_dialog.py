...
class AnyKeycodeDialog(QDialog):
    ...
    def __init__(self, initial):
        ...
        self.setWindowTitle(tr("AnyKeycodeDialog", "输入任意键码"))
    ...
    def on_change(self):
        ...
        if not text:
            self.value = ""
            self.lbl_computed.setText(tr("AnyKeycodeDialog", "输入表达式"))
        elif err:
            self.value = ""
            self.lbl_computed.setText(tr("AnyKeycodeDialog", "输入无效: {}").format(err))
        elif isinstance(value, int):
            self.value = Keycode.serialize(value)
            self.lbl_computed.setText(tr("AnyKeycodeDialog", "计算值: 0x{:X}").format(value))
        else:
            self.value = ""
            self.lbl_computed.setText(tr("AnyKeycodeDialog", "无效输入"))
